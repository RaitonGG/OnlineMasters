...
class Elite_Business_Customizer_Utilities {
	/**
	...
	public static function register_option( $option ) {
		...
		$defaults = apply_filters( 'elite_business_customizer_defaults', array() );
	...
	public static function get_posts_as_array( $args ) {
		...
		if ( ! empty( $get_posts ) ) {
			foreach ( $get_posts as $get_post ) {
				...
				if ( empty( $title ) ) {
					$title = sprintf( esc_html__( 'ID: %s No Title', 'elite-business' ), $get_post->ID );
				}
	...
	static function section_visibility() {
		$options = array(
			...
			'homepage'     => esc_html__( 'Homepage / Frontpage', 'elite-business' ),
			'entire-site'  => esc_html__( 'Entire Site', 'elite-business' ),
		);

		return apply_filters( 'elite_business_section_visibility_options', $options );
	}
}
 ...
 * Initialize class
 */
$elite_business_utilities = new Elite_Business_Customizer_Utilities();
