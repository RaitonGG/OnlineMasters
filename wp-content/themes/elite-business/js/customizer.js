...
jQuery( document ).ready(function($) {
	...
	$('#accordion-section-elite_business_ss_main_content h3').unbind("click");
		...
		if(numRepeaterItems > 0) {
			...
			if(numRepeaterItems > 1) {
				...
				for (i = 1; i < numRepeaterItems; ++i) {
					eliteBusinessAppendRow($(this), defaultValuesArray[i]);
				}
			}
		}
	});
	...
	$(this).find('.sortable_repeater.sortable').sortable({
		update: function(event, ui) {
			eliteBusinessGetAllInputs($(this).parent());
		}
	});
	...
	$('.sortable_repeater.sortable').on('click', '.customize-control-sortable-repeater-delete', function(event) {
		...
		if(numItems > 1) {
			$(this).parent().slideUp('fast', function() {
				...
				$(this).remove();
				eliteBusinessGetAllInputs(parentContainer);
			})
		}
		else {
			$(this).parent().find('.repeater-input').val('');
			eliteBusinessGetAllInputs($(this).parent().parent().parent());
		}
	});
	...
	$('.customize-control-sortable-repeater-add').on( 'click', function(event) {
		event.preventDefault();
		eliteBusinessAppendRow($(this).parent(), '');
		eliteBusinessGetAllInputs($(this).parent());
	});
	...
	function eliteBusinessAppendRow($element, defaultValue) {
	...
	function eliteBusinessGetAllInputs($element) {
	...
	$('.multi-image-checkbox').on('change', function () {
	  eliteBusinessGetAllImageCheckboxes($(this).parent().parent());
	});

	// Get the values from the checkboxes and add to our hidden field
	function eliteBusinessGetAllImageCheckboxes($element) {
	...
	$('.customize-control-tinymce-editor').each(function(){
		...
		var tinyMCEToolbar1String = _wpCustomizeSettings.controls[$(this).attr('id')].eliteBusinesstinymcetoolbar1;
		var tinyMCEToolbar2String = _wpCustomizeSettings.controls[$(this).attr('id')].eliteBusinesstinymcetoolbar2;
		var tinyMCEMediaButtons = _wpCustomizeSettings.controls[$(this).attr('id')].eliteBusinessmediabuttons;
 ...
 */
( function( $, api ) {
	api.sectionConstructor['elite-business-upsell'] = api.Section.extend( {
