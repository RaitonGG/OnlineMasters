/* global eliteBusinessScreenReaderText */
/**
...
(function($) {
	...
	function initMainNavigation(container) {
		...
		var dropdownToggle = $('<button />', {
			...
			.append(eliteBusinessScreenReaderText.icon)
			.append($('<span />', {
				'class': 'screen-reader-text',
				text: eliteBusinessScreenReaderText.expand
			}));
		...
		container.find('.current-menu-ancestor > button')
			...
			.text(eliteBusinessScreenReaderText.collapse);
		...
		container.find('.dropdown-toggle').on( 'click', function(e) {
			...
			// jscs:enable
			screenReaderSpan.text(screenReaderSpan.text() === eliteBusinessScreenReaderText.expand ? eliteBusinessScreenReaderText.collapse : eliteBusinessScreenReaderText.expand);
		});
	}
	...
	(function() {
		...
		menuToggle.on('click.eliteBusiness', function() {
	...
	(function() {
		...
		function toggleFocusClassTouchScreen() {
			if (window.innerWidth >= 910) {
				$(document.body).on('touchstart.eliteBusiness', function(e) {
					if (!$(e.target).closest('.main-navigation li').length) {
						$('.main-navigation li').removeClass('focus');
					}
				});
				siteNavigation.find( '.menu-item-has-children > a, .page_item_has_children > a' ).on( 'touchstart.eliteBusiness', function( e ) {
				...
				siteNavigation.find('.menu-item-has-children > a').unbind('touchstart.eliteBusiness');
			}
		}

		if ('ontouchstart' in window) {
			$(window).on('resize.eliteBusiness', toggleFocusClassTouchScreen);
			toggleFocusClassTouchScreen();
		}

		siteNavigation.find('a').on('focus.eliteBusiness blur.eliteBusiness', function() {
	...
	$(document).ready(function() {
        $(window).on('load.eliteBusiness resize.eliteBusiness', function() {
            if ( window.innerWidth < 910 ) {
