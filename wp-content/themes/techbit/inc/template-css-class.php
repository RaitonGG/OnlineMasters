 ...
 * @return array
 */
function techbit_body_classes( $classes ) {
    $post = get_post();
    ...
    if ( is_archive() || is_home() ) {
        $archive_sidebar_layout = get_theme_mod( 'techbit_archive_sidebar_layout', 'no-sidebar' );
        $classes[] = esc_attr( $archive_sidebar_layout );
    } elseif( is_single() ) {
        $single_post_sidebar_layout = get_post_meta( $post->ID, 'techbit_post_sidebar_layout', true );
        if ( 'layout--default-sidebar' !== $single_post_sidebar_layout && !empty( $single_post_sidebar_layout ) ) {
            ...
            $posts_sidebar_layout = get_theme_mod( 'techbit_posts_sidebar_layout', 'right-sidebar' );
            $classes[] = esc_attr( $posts_sidebar_layout );
        }
    } elseif( is_page() ) {
        $single_page_sidebar_layout = get_post_meta( $post->ID, 'techbit_post_sidebar_layout', true );
        if ( 'layout--default-sidebar' !== $single_page_sidebar_layout && !empty( $single_page_sidebar_layout ) ) {
            ...
            $pages_sidebar_layout = get_theme_mod( 'techbit_pages_sidebar_layout', 'right-sidebar' );
            $classes[] = esc_attr( $pages_sidebar_layout );
        }
    }
    return $classes;
}
add_filter( 'body_class', 'techbit_body_classes' );